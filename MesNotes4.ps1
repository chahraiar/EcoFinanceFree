...
# Version: 2.0 - Correction problème module expo-font
...
# Installation des dépendances de navigation compatibles
Write-Host "🧭 Installation des dépendances de navigation compatibles..." -ForegroundColor Cyan
npm install --save @react-navigation/native@6.1.9 @react-navigation/bottom-tabs@6.5.11 --legacy-peer-deps
...
# Installation d'expo-font en version spécifique compatible
Write-Host "🖋️ Installation d'expo-font en version compatible..." -ForegroundColor Cyan
npm install --save expo-font@~11.4.0 --legacy-peer-deps
...
# Création d'un fichier metro.config.js pour résoudre les problèmes de Metro
...
# Création d'un exemple d'App.js minimaliste
  ...
  useEffect(() => {
    async function prepare() {
      try {
        ...
        // Vous pouvez charger des polices ici si nécessaire
...
Write-Host ""
Write-Host "🚨 IMPORTANT : Avant de démarrer, exécutez ces commandes:" -ForegroundColor Red
Write-Host "   npx expo install" -ForegroundColor Yellow
Write-Host "   npx expo install --check" -ForegroundColor Yellow
Write-Host ""
